...
public:
  ...
  virtual void setPosition(units::radian_t position) {
    units::radian_t tagrePosition =
        std::clamp(position * inversion, minPosition, maxPosition);
   ...
   */
  virtual units::radian_t getMaxPosition() const { return maxPosition; }
   ...
   */
  virtual void setInverted(bool isInverted) { inversion = isInverted ? -1 : 1; }
   ...
   */
  virtual bool getInverted() const { return inversion == 1; }

  /**
   * Common interface for disabling a mechanism.
   */
  virtual void disable() { servo.SetDisabled(); }

  /**
   * Common interface to stop the mechanism until `setPosition` is called again.
   */
  virtual void stop() { servo.SetDisabled(); }
