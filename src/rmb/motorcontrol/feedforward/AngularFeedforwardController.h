...
/**
 * Interface for setting a motor controllers angular velocity using a
 * feedfoward. <b>Beware<\b> that since there is no feedbakc device several
 * functions will behave incorrectly. Additionaly, an `update` method may need
 * to be added for proper voltage compenstation.
 */
...
public:
   ...
   */
  void setMaxVelocity(units::radians_per_second_t max) { maxVelocity = max; }
   ...
   */
  units::radians_per_second_t getMaxVelocity() const { return maxVelocity; }
...
private:
  ...
  units::radians_per_second_t maxVelocity =
      units::radians_per_second_t(std::numeric_limits<double>::infinity());
};
} // namespace rmb
